...
import CalculatedEntryForm from '../components/forms/CalculatedEntryForm';
import {
...
const CalculatedEntryPage: React.FC = () => {
  ...
  const handleSimulate = () => {
    ...
    const variantErrors = validateVariant2(formData);
    ...
    if (Object.keys(allErrors).length === 0) {
      ...
      const results = calculateResults(formData, 'calculated');
  ...
  const handleInputChange = (field: keyof InputParameters, value: any) => {
    ...
    // Synchronisation automatique du nombre de trades avec les pourcentages de baisse
    if (field === 'dropPercentages' && Array.isArray(value)) {
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white dark:bg-slate-900 p-8 rounded-lg border border-gray-300 dark:border-slate-700 shadow-lg">
      <h2 className="text-3xl font-bold mb-8 text-center">{t('tabs.calculated')}</h2>
...
export default CalculatedEntryPage; 