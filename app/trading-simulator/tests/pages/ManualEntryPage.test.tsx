...
import { ManualEntryPage } from '../../src/pages';
...
vi.mock('../../src/components/forms/ManualEntryForm', () => ({
  default: ({ formData, errors, handleInputChange }) => (
    <div data-testid="manual-entry-form-mock">
      <button 
        data-testid="entry-form-change" 
        onClick={() => handleInputChange('entryPrices', [100, 90])}
      >
        Ajouter prix
      </button>
      {errors.entryPrices && <span data-testid="error-entry-prices">{errors.entryPrices}</span>}
    </div>
  )
}));
...
const validateVariant1Mock = vi.fn().mockImplementation(() => ({}));
...
vi.mock('../../src/utils', () => ({
  ...
  validateVariant1: (formData) => validateVariant1Mock(formData),
...
describe('ManualEntryPage Component', () => {
  beforeEach(() => {
    ...
    validateVariant1Mock.mockClear();
  ...
  test('rend les composants de formulaire nécessaires', () => {
    ...
    expect(screen.getByTestId('manual-entry-form-mock')).toBeInTheDocument();
  ...
  test('simule correctement et affiche les résultats', () => {
    ...
    expect(validateVariant1Mock).toHaveBeenCalled();
    ...
    expect(screen.getByTestId('entry-prices')).toHaveTextContent('100');
  });
  ...
  test('gère correctement les avertissements de levier et durée', () => {
    ...
    const ManualEntryPageMock = () => {
      const formData = { 
        ...
        recovery: false, symbol: 'BTC/USDT', numberOfTrades: 1, entryPrices: [100],
        initialEntryPrice: 100, dropPercentage: 50, dropPercentages: []
      };
    ...
    // Remplacer temporairement le composant ManualEntryPage
    vi.doMock('../../src/pages', () => ({
      ManualEntryPage: ManualEntryPageMock
    }));
    
    render(<ManualEntryPageMock />);
  ...
  test('a les classes CSS appropriées', () => {
    const { container } = render(<ManualEntryPage />);
